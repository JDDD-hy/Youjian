...
connection_unconfirmed | reconnected
```

只允许服务端函数追加，禁止任何 UPDATE 和 DELETE。自动事件的 `actor_id` 可为空，原因写入 `metadata`。
...
- daily：房间时区下一个本地自然日 00:00；
- weekly：房间时区下一个周一 00:00；
- monthly：房间时区下月 1 日 00:00。
