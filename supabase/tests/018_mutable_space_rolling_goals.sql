...
reset role;
select is((select tier from public.achievements where space_id='10000000-0000-0000-0000-000000000181' and achievement_type='goal_milestone' and dedupe_key='goal:'||(select id::text from public.goals where space_id='10000000-0000-0000-0000-000000000181' and status='completed' order by completed_at desc limit 1)),'bronze','first completed goal earns bronze tier');
select ok((select participants_recorded from public.achievements where space_id='10000000-0000-0000-0000-000000000181' and achievement_type='goal_milestone' and dedupe_key='goal:'||(select id::text from public.goals where space_id='10000000-0000-0000-0000-000000000181' and status='completed' order by completed_at desc limit 1)),'goal achievement freezes participant provenance');
select is((select count(*)::int from public.achievement_participants ap join public.achievements a on a.id=ap.achievement_id where a.space_id='10000000-0000-0000-0000-000000000181' and a.achievement_type='goal_milestone' and a.dedupe_key='goal:'||(select id::text from public.goals where space_id='10000000-0000-0000-0000-000000000181' and status='completed' order by completed_at desc limit 1)),2,'goal achievement records the accepted participant snapshot');
