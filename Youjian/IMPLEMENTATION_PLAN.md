...
- [x] 本地一条命令启动前端；
- [x] 本地 Supabase 可以初始化和重建；
- [x] 空页面在 320px 和桌面尺寸无横向滚动；
- [x] CI 全绿；
- [x] `.env.example` 完整且无密钥。
...
- [x] 所有公开表启用 RLS；
- [x] 直接 INSERT/UPDATE/DELETE 核心表被拒绝；
- [x] 安全函数的 search_path 已固定；
- [x] 跨用户、跨房间权限测试通过；
- [x] 数据库 reset 后测试可重复通过。
...
- [x] API 响应不进入 Service Worker 缓存；
- [x] 房主和成员看到正确管理入口；
- [x] 核心流程通过键盘和屏幕阅读器 smoke test；
- [x] 320px 无关键横向滚动。
